...
- **Modal Layering**: Keep `Dialog` and `Sheet` backdrop/content z-index values wired through `src/lib/runtime/dialog/layers.ts` so nested overlays (dialog-in-sheet, sheet-in-dialog, and same-family nesting) stack in opening order without API changes.
